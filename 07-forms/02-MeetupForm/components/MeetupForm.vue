...
function createAgendaItem(lastAgenda = null) {
  let startsAt
  if (lastAgenda) {
    startsAt = lastAgenda.endsAt
  }
  else {
    startsAt = '00:00'
  }
...
export default {
  ...
  methods: {
    addAgendaItem() {
      let lastAgenda = this.localMeetup.agenda[this.localMeetup.agenda.length - 1]
      if (lastAgenda) {
        this.localMeetup.agenda.push(createAgendaItem(lastAgenda))
      } else {
